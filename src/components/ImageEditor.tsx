...
import { Box, Button, Grid, Typography, Slider, FormControl, InputLabel, Select, MenuItem, SelectChangeEvent, Paper } from '@mui/material'
...
const ImageEditor = ({ imageUrl, locationData, onImageEdit }: ImageEditorProps) => {
  ...
  const annotateImage = async () => {
    ...
    try {
      ...
      const textWidth = Math.max(...locationText.map(line => ctx.measureText(line).width))
      ...
      alert('Failed to annotate image')
    } finally {
      setLoading(false)
    }
  }
